...
This is the third tutorial in the series [Data Visualization With R](https://tutorials.rsquaredacademy.com/data-visualization/index.html). In the previous tutorial, we learned how to add title, subtitle and axis labels. We also learned how to modify the range of the axis. In this tutorial, we will learn how to create scatter plots. 
...
All the data sets used in this tutorial can be found [here](https://github.com/rsquaredacademy/datasets) and code can be downloaded from [here](https://gist.github.com/rsquaredacademy/f5cd1315826bd44466a15444456ea61d).

### Basic Plot

Let us recreate the plot that we had created in the first tutorial by using the `mtcars` data set. We will use the `disp` (displacement) and `mpg` (miles per gallon) variables. `disp` will be on the X axis and `mpg` will be on the Y axis.

```{r scatter1, fig.align='center', fig.height=5, fig.width=5}
plot(mtcars$disp, mtcars$mpg)
```

We have created a very basic plot and any one looking at it for the first time will get confused with the axis labels `mtcars$disp` and `mtcars$mpg`. Let us put into practice what we learnt in the second tutorial, and add a title to the plot, and make the axis labels more meaningful.
